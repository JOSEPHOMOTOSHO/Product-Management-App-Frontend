...
async function postData({userName, email, phone, address, password, location}) {
    console.log('going')
    const result = await axios({ method:'post', url:`${process.env.REACT_APP_BASE_URL}users/register`, data:{ userName, email, phoneNumber:phone, location, address, password}})
    console.log(result)
    return result.data
  }

export const Signup = ({show, setShow}) => {
    console.log(process.env.REACT_APP_GOOGLE_API_KEY, 'key' , process.env.REACT_APP_BASE_URL, 'LOP')
    ...
    async function addNewProduct () {
        try{
            console.log('hello')
            const result = await postData({ userName:name, email, phone, location:[coord.lng, coord.lat], address, password})
            console.log(result)
            setSuccess(true)
        }catch(error){
      ...
      { show && (<div className="body">
                ...
                {success && (<h4 style={{color:'green'}}>Registration successful you can now login <u style={{cursor:'pointer'}} onClick={()=>history('/login')}>here</u></h4>)}
                    <form>
                        ...
                        <div className="user-box">
                            <input type="text" value={phone} onChange={(e)=>{setPhone(e.target.value); setError(false)}} />
